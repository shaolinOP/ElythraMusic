...
class ImportExportService {
  ...
  static Future<bool> isPlaylistExists(String playlistName) async {
    ...
    final _list = await ElythraDBService.getPlaylists4Library();
  ...
  static Future<String?> exportPlaylist(String playlistName,
    ...
    final mediaPlaylistDB = await ElythraDBService.getPlaylist(playlistName);
    if (mediaPlaylistDB != null) {
      try {
        // Fetch all media items in the playlist.
        List<MediaItemDB>? playlistItems =
            await ElythraDBService.getPlaylistItems(mediaPlaylistDB);
        ...
        if (playlistItems != null) {
          ...
          final Map<String, dynamic> playlistMap = {
            '_meta': {
              'generated_by':
                  'Elythra - Open Source Music Streaming Application',
              'version':
                  'v${packageInfo.version}+${int.parse(packageInfo.buildNumber) % 1000}',
              'exportedAt': DateTime.now().toIso8601String(),
              'note':
                  'This file is automatically generated by Elythra and is intended solely for importing playlists within the application. Manual modification of this file is strongly discouraged, as it may result in errors, data inconsistency, or failed imports. For support or more information, please visit: https://github.com/HemantKArya/ElythraTunes.',
            },
          ...
          final path = await writeToJSON(
              '${mediaPlaylistDB.playlistName}_ElythraPlaylist.json',
              playlistMap,
              path: filePath);
  ...
  static Future<String?> exportMediaItem(MediaItemDB mediaItemDB) async {
    try {
      ...
      final packageInfo = await PackageInfo.fromPlatform();
      mediaItemMap['_meta'] = {
        'generated_by': 'Elythra - Open Source Music Streaming Application',
        'version':
            'v${packageInfo.version}+${int.parse(packageInfo.buildNumber) % 1000}',
        'exportedAt': DateTime.now().toIso8601String(),
        'note':
      ...
      final path = await writeToJSON(
          '${mediaItemDB.title}_ElythraSong.json', mediaItemMap);
      ...
      await readFromJSON(filePath).then((playlistMap) async {
        if (playlistMap != null && playlistMap.isNotEmpty) {
          ...
          for (final mediaItemMap in playlistMap['mediaItems']) {
            ...
            await ElythraDBService.addMediaItem(mediaItemDB, playlistName);
    ...
    try {
      ...
      await readFromJSON(filePath).then((mediaItemMap) {
        if (mediaItemMap != null && mediaItemMap.isNotEmpty) {
          final mediaItemDB = MediaItemDB.fromMap(mediaItemMap);
          ElythraDBService.addMediaItem(mediaItemDB, "Imported");
