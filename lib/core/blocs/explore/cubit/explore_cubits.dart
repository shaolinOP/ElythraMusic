...
class RecentlyCubit extends Cubit<RecentlyCubitState> {
  ...
  void getRecentlyPlayed() async {
    final mediaPlaylist = await ElythraDBService.getRecentlyPlayed(limit: 15);
...
class ChartCubit extends Cubit<ChartState> {
  ...
  Future<void> getChartFromDB() async {
    final chart = await ElythraDBService.getChart(chartInfo.title);
    if (chart != null) {
      emit(state.copyWith(
...
class YTMusicCubit extends Cubit<YTMusicCubitState> {
  ...
  void fetchYTMusicDB() async {
    final data = await ElythraDBService.getAPICache("YTMusic");
    if (data != null) {
  ...
  Future<void> fetchYTMusic() async {
    ...
    if (ytCharts.isNotEmpty) {
      ...
      ElythraDBService.putAPICache("YTMusic", ytChartsJson);
